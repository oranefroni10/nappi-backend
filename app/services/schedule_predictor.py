                    ...
                    try:
                        ...
                        if pattern_hour > current_hour:
                            ...
                            return SleepPrediction(
                                predicted_start=predicted_start,
                                confidence="medium",
                                prediction_type="nap" if pattern.get("label", "").lower() != "night sleep" else "bedtime",
    ...
    def _generate_suggestions(
        ...
        if prediction:
            if prediction.confidence == "high":
                suggestions.append(f"Start sleep routine 15-20 minutes before {prediction.predicted_start.strftime('%I:%M %p')}")
            elif prediction.prediction_type == "bedtime":
                suggestions.append(f"Begin calming bedtime routine around {prediction.predicted_start.strftime('%I:%M %p')}")
